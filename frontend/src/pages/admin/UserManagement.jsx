  ...
  useEffect(() => {
    ...
    if (userToEdit) {
      setFormData({
        ...
        dob: userToEdit.student?.dob
          ? new Date(userToEdit.student.dob).toISOString().split("T")[0]
          : "",
        phone_number:
          userToEdit.tutor?.phone_number ||
          userToEdit.parents?.phone_number ||
          "",
...
const UserManagement = () => {
  ...
  const fetchAllUsers = useCallback(async () => {
    ...
    try {
      ...
      setError(
        `Lỗi tải danh sách: ${err.response?.data?.message || err.message}`
      );
      setAllUsers([]);
    } finally {
      setLoading(false);
  ...
  const handleToggleStatusAction = async () => {
    ...
    try {
      ...
      setError(
        `Lỗi cập nhật trạng thái: ${err.response?.data?.message || err.message}`
      );
    }
  };
