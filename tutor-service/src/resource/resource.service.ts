...
export class FolderService {
    ...
    return this.prisma.$transaction(async (tx) => {
      const newFolder = await tx.folders.create({
        data: {
          ...
          ...(data.parent_id
            ? { Folders: { connect: { folder_id: data.parent_id } } }
            : {}),
        },
      });
  ...
  async traverseFolderTree(class_id: string, parent_id: string | null = null) {
    ...
    for (const item of fatherLayer) {
      const children: any = await this.traverseFolderTree(
        class_id,
        item.folder_id,
      );
...
export class ResourceService {
  constructor(
    ...
    private readonly configService: ConfigService, // THÊM CONFIGSERVICE Ở ĐÂY
  ) {}
  ...
  async createNewDocs(
    ...
    const checkExisted = await this.prisma.folders.findFirst({
      where: { folder_id },
    });
    ...
    if (!fileStream) {
      throw new InternalServerErrorException(
        'File buffer not found. Check Multer configuration.',
      );
    }
  ...
  async getPresignedUrl(did: string) {
    const fileInfo = await this.prisma.resources.findUnique({ where: { did } });
    ...
    // Tạo URL có hiệu lực trong 15 phút (900 giây)
    // Lưu ý: Đảm bảo "this.s3.client" được cấu hình public trong class S3Service của bạn
    const signedUrl = await getSignedUrl(this.s3.client, command, {
      expiresIn: 900,
    });
    return { signedUrl, fileInfo };
  }
  ...
  async deleteDocs(did: string) {
    ...
    return this.prisma.$transaction(async (tx) => {
      if (fileInfo.file_path && fileInfo.file_path.includes('amazonaws.com')) {
        const url = new URL(fileInfo.file_path);
        const key = decodeURIComponent(url.pathname.substring(1));
        await this.s3.deleteFiles([key]);
      }
